  ...
  <div class="order-detail-page">
    ...
    <template v-else-if="detail">
      ...
      <div class="admin-card">
        ...
        <el-descriptions :column="3" border>
          <el-descriptions-item label="订单号" :span="2">
            <span style="font-family: monospace; font-weight: 600">{{ detail.orderNo }}</span>
          </el-descriptions-item>
          <el-descriptions-item label="创建时间">
            {{ formatTime(detail.createdTime) }}
          </el-descriptions-item>
          <el-descriptions-item label="套餐类型">
            <el-tag :type="PlanTypeMap[detail.planType as PlanType]?.type" size="small">
              {{ PlanTypeMap[detail.planType as PlanType]?.label || detail.planType }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="套餐名称">{{ detail.planName }}</el-descriptions-item>
          <el-descriptions-item label="金额">
            <span style="font-weight: 700; color: #f5222d">{{ formatAmount(detail.amount) }}</span>
          </el-descriptions-item>
          <el-descriptions-item label="支付方式">
            {{ PayMethodMap[detail.payMethod as PayMethod]?.label || detail.payMethod }}
          </el-descriptions-item>
          <el-descriptions-item label="支付状态">
            <el-tag :type="PayStatusMap[detail.payStatus as PayStatus]?.type" size="small">
              {{ PayStatusMap[detail.payStatus as PayStatus]?.label }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="退款状态">
            <el-tag :type="RefundStatusMap[detail.refundStatus as RefundStatus]?.type" size="small">
              {{ RefundStatusMap[detail.refundStatus as RefundStatus]?.label }}
            </el-tag>
          </el-descriptions-item>
        ...
        <div class="user-info-row">
          <div class="user-info-item">
            ...
            <el-button
              type="primary"
              link
              @click="router.push(`/accounts/${detail.accountId}`)"
            >{{ detail.accountPhone }}</el-button>
          </div>
          <div class="user-info-item" v-if="detail.accountNickname">
            <span class="label">昵称：</span>
            <span>{{ detail.accountNickname }}</span>
          </div>
          <div class="user-info-item">
      ...
      <div class="admin-card">
        ...
        <el-table
          v-if="detail.refundRecords && detail.refundRecords.length"
          :data="detail.refundRecords"
          stripe
          style="width: 100%"
        >
          ...
          <el-table-column label="处理时间" width="160">
            <template #default="{ row }">{{ formatTime(row.processedTime) }}</template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog v-model="refundDialog.visible" title="发起退款" width="500px" :close-on-click-modal="false">
      <el-alert type="error" show-icon :closable="false" style="margin-bottom: 16px">
        ...
        <template #default>
          订单号：{{ detail?.orderNo }}，原始金额：{{ formatAmount(detail?.amount) }}
        </template>
      </el-alert>
...
const canRefund = computed(() => {
  ...
  return (
    detail.value.payStatus === PayStatus.PAID &&
    [RefundStatus.NONE, RefundStatus.REJECTED].includes(detail.value.refundStatus as RefundStatus)
  )
})
...
function handleRefund() {
  refundDialog.refundAmount = detail.value?.amount || 0
  refundDialog.reason = ''
