    ...
    <div class="admin-card" style="padding: 0">
      ...
      <el-table v-loading="loading" :data="tableData" stripe style="width: 100%" row-key="id">
        ...
        <el-table-column label="目标用户" width="100">
          <template #default="{ row }">
            {{ NoticeTargetUsersMap[row.target_users as NoticeTargetUsers]?.label || row.target_users }}
          </template>
        </el-table-column>
    ...
    >
      <el-form :model="formDialog" :rules="formRules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="目标用户" prop="target_users">
          <el-select v-model="formDialog.target_users" style="width: 100%">
            <el-option v-for="(v, k) in NoticeTargetUsersMap" :key="k" :label="v.label" :value="k" />
          </el-select>
        </el-form-item>
        <el-form-item label="定时发布">
          <el-date-picker
            v-model="formDialog.publish_time"
            type="datetime"
            placeholder="留空=立即发布"
...
async function handlePublish(row: NoticeItem) {
  const targetLabel = NoticeTargetUsersMap[row.target_users as NoticeTargetUsers]?.label || row.target_users
...
const formDialog = reactive({
  ...
  target_users: NoticeTargetUsers.ALL,
  publish_time: '',
  content: '',
  loading: false,
})
...
function openCreateDialog() {
  Object.assign(formDialog, {
    ...
    title: '', type: NoticeType.NOTICE, target_users: NoticeTargetUsers.ALL,
...
function openEditDialog(row: NoticeItem) {
  Object.assign(formDialog, {
    ...
    title: row.title, type: row.type, target_users: row.target_users,
    publish_time: row.publish_time || '', content: row.content,
  })
}
...
async function confirmForm() {
  ...
  try {
    const data = {
      ...
      target_users: formDialog.target_users,
      content: formDialog.content,
      publish_time: formDialog.publish_time || undefined,
    }
